...
var Webcam = {
	...
	uploadPlus: function(image_data_uri, transType, userId, transTime, transDate, target_url, callback) {
		// submit image data to server using binary AJAX
		if (callback) Webcam.on('uploadComplete', callback);
		var form_elem_name = 'webcam';
		...
		// customize by john paul del mundo for special case and usability
		var nTransType = "transType";
		var nUserId = "userId";
		var nTransTime = "transTime";
		var nTransDate = "transDate";

		form.append( form_elem_name, blob, form_elem_name+"."+image_fmt.replace(/e/, '') );
		form.append( nTransType, transType );
		form.append( nUserId, userId );
		form.append( nTransTime, transTime );
		form.append( nTransDate, transDate );

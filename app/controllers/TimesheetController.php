...
class TimesheetController extends \BaseController {
	...
	public function store()
	{
		...
		$to = Input::get('dtrto');
		if ($from == "" || $to == "") return Redirect::to('timesheet')->with('message', 'warning::Date from and to are required!')->withInput();
		...
		if (Input::hasFile('dtr'))
		{
			...
			if ($type === 'text/plain')
			{
				...
				return Redirect::to('timesheet')->with('message', 'success::Upload complete!');
			}

			return Redirect::to('timesheet')->with('message', 'warning::Invalid file type.')->withInput();
		}

		return Redirect::to('timesheet')->with('message', 'warning::Please select dtr file.')->withInput();
	}
